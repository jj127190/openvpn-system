...
func init() {
    ...
    //gorm
    gdb, err := gorm.Open("mysql", "root:123123123@(1.2.3.4:3306)/VpnAudit?charset=utf8&parseTime=True&loc=Local")
    if err != nil {
...
func CreateIptablesRuls(res []string) {
    ...
    for _, val := range currips {
        ...
        // iptables -I FORWARD 1 -s  vpn 分配的ip  -i tun0 -d 域名ip -j ACCEPT
